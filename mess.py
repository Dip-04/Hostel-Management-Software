...
class Mess:
    ...
    def __init__(self,root6):
        ...
        btf = Frame(self.root6, bd=3, relief=RIDGE)
        btf.place(x=570, y=540, width=200, height=50)
        Button(btf, text='Update', font=('arial', 15, 'bold'), bg='black', fg='gold', width=15,command=lambda :self.UPLOAD()).grid(row=0, column=0)
    ...
    def UPLOAD(self):
     try:
        file=filedialog.askopenfilename(parent=self.root6)
        self.mess.set(file)
        nen = ttk.Entry(self.root6, font=('arial', 10, 'bold'), width=25, textvariable=self.mess, state="readonly")
        nen.place(x=4455,y=5126)
        file1=open("mess.txt",'w')
        file1.write(file)
        file1.close()
